...
llama_sampling_context * llama_sampling_init(const gpt_params & params) {
    ...
    result->params.n_prev = params.n_ctx;
    result->grammar = nullptr;

    // if there is a grammar, parse it
    if (params.sparams.grammar[0]) {
        result->parsed_grammar = grammar_parser::parse(params.sparams.grammar);
...
static llama_token llama_sampling_sample_impl(
    ...
    // apply proper biases
    for (auto &&i : ctx_sampling->biases) {
        switch (i.op) {
